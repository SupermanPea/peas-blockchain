...
from peas.daemon.keychain_proxy import (
    KeychainProxy,
...
from peas.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from peas.protocols import wallet_protocol
from peas.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from peas.protocols.protocol_message_types import ProtocolMessageTypes
from peas.protocols.wallet_protocol import (
...
from peas.server.node_discovery import WalletPeers
...
from peas.util.ints import uint32, uint128
from peas.util.keychain import Keychain
from peas.util.lru_cache import LRUCache
from peas.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from peas.wallet.settings.settings_objects import BackupInitialized
from peas.wallet.transaction_record import TransactionRecord
from peas.wallet.util.backup_utils import open_backup_file
from peas.wallet.util.wallet_types import WalletType
from peas.wallet.wallet_action import WalletAction
from peas.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'peas keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSPeasConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSPeasConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSPeasConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSPeasConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSPeasConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSPeasConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
