...
from peas.wallet.puzzles.puzzle_utils import (
...
from peas.wallet.secret_key_store import SecretKeyStore
from peas.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_peas(self, peas_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if peas_amount < 0:
            utxos = await self.select_coins(abs(peas_amount), exclude)
        else:
        ...
        peas_amount = spend_value + peas_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": peas_amount}]
