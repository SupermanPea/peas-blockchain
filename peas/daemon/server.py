...
from peas.cmds.init_funcs import check_keys, peas_init
from peas.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from peas.daemon.keychain_server import KeychainServer, keychain_commands
from peas.daemon.windows_signal import kill
from peas.server.server import ssl_context_for_root, ssl_context_for_server
from peas.ssl.create_ssl import get_mozilla_ca_crt
...
from peas.util.keychain import (
    Keychain,
...
from peas.util.service_groups import validate_service
from peas.util.setproctitle import setproctitle
from peas.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Peas.")
    quit()
...
service_plotter = "peas plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "peas": "peas",
        "peas_wallet": "start_wallet",
        "peas_full_node": "start_full_node",
        "peas_harvester": "start_harvester",
        "peas_farmer": "start_farmer",
        "peas_introducer": "start_introducer",
        "peas_timelord": "start_timelord",
        "peas_timelord_launcher": "timelord_launcher",
        "peas_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("peas daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered PEAS_ROOT
    os.environ["PEAS_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with PEAS_ROOT: {os.environ['PEAS_ROOT']}")
    ...
    if service_command == "peas_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in peas_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    peas_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("peas_daemon")
