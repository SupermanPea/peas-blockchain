...
ROOT = pathlib.Path(importlib.import_module("peas").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `peas` file name (which is the executable)
    and the `peas` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `peas` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("peas-blockchain"))[0]
...
# TODO: collapse all these entry points into one `peas_exec` entrypoint that accepts the server as a parameter

entry_points = ["peas.cmds.peas"] + [f"peas.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "peas.util.bip39"])

if THIS_IS_WINDOWS:
    peas_mod = importlib.import_module("peas")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/peas/util/english.txt", "peas/util"))
datas.append((f"{ROOT}/peas/util/initial-config.yaml", "peas/util"))
datas.append((f"{ROOT}/peas/wallet/puzzles/*.hex", "peas/wallet/puzzles"))
datas.append((f"{ROOT}/peas/ssl/*", "peas/ssl"))
...
add_binary("peas", f"{ROOT}/peas/cmds/peas.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/peas/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/peas/server/start_{server}.py", COLLECT_ARGS)
