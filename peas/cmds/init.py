...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.peas/[version]/config/ssl/ca
    - Shut down all peas daemon processes with `peas stop all -d`
    - Run `peas init -c [directory]` on your remote harvester,
      where [directory] is the the copy of your Farming Machine CA directory
    - Get more details on remote harvester on Peas wiki:
      https://github.com/Peas-Network/peas-blockchain/wiki/Farming-on-many-machines
    """
    from pathlib import Path
    from .init_funcs import init
    from peas.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import peas_init
    from peas.util.default_root import DEFAULT_ROOT_PATH

    peas_init(DEFAULT_ROOT_PATH)
