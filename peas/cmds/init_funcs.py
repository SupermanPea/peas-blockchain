...
from peas.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from peas.util.config import (
    create_default_peas_config,
    initial_config_file,
    load_config,
    save_config,
...
from peas.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'peas keys generate'")
        return None
    ...
    if "pea_target_address" not in config["farmer"]:
        print(f"Setting the pea destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "pea_target_address" not in config["pool"]:
        print(f"Setting the pea destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root_path: Path):
    ...
    peas_ca_crt, peas_ca_key = get_peas_ca_crt_key()
    peas_ca_crt_path = ca_dir / "peas_ca.crt"
    peas_ca_key_path = ca_dir / "peas_ca.key"
    write_ssl_cert_and_key(peas_ca_crt_path, peas_ca_crt, peas_ca_key_path, peas_ca_key)
    ...
    peas_ca_crt, peas_ca_key = get_peas_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, peas_ca_crt, peas_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if peas_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return peas_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def peas_version_number() -> Tuple[str, str, str, str]:
...
def peas_minor_release_number():
    res = int(peas_version_number()[2])
    print(f"Install release number: {res}")
    return res


def peas_full_version_str() -> str:
    major, minor, patch, dev = peas_version_number()
...
def peas_init(
    ...
    """
    if os.environ.get("PEAS_ROOT", None) is not None:
        print(
            f"warning, your PEAS_ROOT is set to {os.environ['PEAS_ROOT']}. "
            f"Please unset the environment variable and run peas init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Peas directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if PEAS_ROOT is set, or if user has run peas init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'peas keys show --show-mnemonic-seed'")
