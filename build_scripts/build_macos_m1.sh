...
npm install @weed-network/notarize-cli -g
...
electron-packager . Weed --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Weed-darwin-arm64/Weed.app --platform=darwin \
...
mv Weed-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Weed-$WEED_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Weed-darwin-arm64/Weed.app Weed-$WEED_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.weed.blockchain \
...
# xcrun altool --notarize-app -f Weed-0.1.X.dmg --primary-bundle-id net.weed.blockchain -u username -p password
...
# xcrun stapler staple Weed-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Weed-0.1.X.dmg
