...
import { /* mojo_to_weed_string, */ weed_to_mojo } from '../../../util/weed';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_weed_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of weed in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Weed that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Weed that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send weed to coloured address. Please enter a weed
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'weed_addr://') {
    ...
    const amountValue = Number.parseFloat(weed_to_mojo(amount));
    const feeValue = Number.parseFloat(weed_to_mojo(fee));
