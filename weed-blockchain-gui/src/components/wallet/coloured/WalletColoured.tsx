...
import { AlertDialog, Card, Flex } from '@weed/core';
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_weed = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_weed = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_weed = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('weed_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
