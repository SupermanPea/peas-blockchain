...
import { mojo_to_weed } from '../../util/weed';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_weed(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_weed(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const chiaFees = blockRecord.fees
    ? mojo_to_weed(BigInt(blockRecord.fees))
    : '';
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Weed blockchain
            </Trans>
          </BlockTitle>
        }
        action={
