...
const getChiaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('weed');
  // first see if we can get a weed exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or weed is on the path
    try {
      version = child_process
