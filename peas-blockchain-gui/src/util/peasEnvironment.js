...
const getChiaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('peas');
  // first see if we can get a peas exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or peas is on the path
    try {
      version = child_process
