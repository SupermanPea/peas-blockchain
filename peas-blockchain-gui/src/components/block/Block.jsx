...
export default function Block() {
  ...
  const poolReward = mojo_to_peas(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_peas(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const chiaFees = blockRecord.fees
    ? mojo_to_peas(BigInt(blockRecord.fees))
    : '';
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Peas blockchain
            </Trans>
          </BlockTitle>
        }
        action={
