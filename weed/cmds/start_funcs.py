...
from weed.cmds.passphrase_funcs import get_current_passphrase
from weed.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from weed.util.keychain import KeyringMaxUnlockAttempts
from weed.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    weed = sys.argv[0]
    process = subprocess.Popen(f"{weed} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the weed daemon")
        return None
