...
from weed.cmds.configure import configure_cmd
from weed.cmds.farm import farm_cmd
from weed.cmds.init import init_cmd
from weed.cmds.keys import keys_cmd
from weed.cmds.netspace import netspace_cmd
from weed.cmds.passphrase import passphrase_cmd
from weed.cmds.plots import plots_cmd
from weed.cmds.show import show_cmd
from weed.cmds.start import start_cmd
from weed.cmds.stop import stop_cmd
from weed.cmds.wallet import wallet_cmd
from weed.cmds.plotnft import plotnft_cmd
from weed.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from weed.util.keychain import set_keys_root_path, supports_keyring_passphrase
...
@click.group(
    help=f"\n  Manage weed blockchain infrastructure ({__version__})\n",
    epilog="Try 'weed start node', 'weed netspace -d 192', or 'weed show -s'",
...
if not supports_keyring_passphrase():
    from weed.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show weed version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs weed daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by weed start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from weed.daemon.server import async_run_daemon
