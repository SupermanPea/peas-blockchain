...
from weed.cmds.units import units
...
from weed.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total weed farmed: {amounts['farmed_amount'] / units['weed']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['weed']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['weed']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'weed start wallet' and 'weed wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'weed wallet show'")
    else:
        print("Note: log into your key using 'weed wallet show' to see rewards for each key")
