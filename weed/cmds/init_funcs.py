...
from weed.util.config import (
    create_default_weed_config,
    initial_config_file,
    load_config,
    save_config,
...
from weed.util.keychain import Keychain
from weed.util.path import mkdir
from weed.util.ssl import (
...
from weed.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'weed keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "pea_target_address" not in config["farmer"]
    stop_searching_for_pool = "pea_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("pea_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("pea_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "pea_target_address" not in config["farmer"]:
        print(f"Setting the wee destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["pea_target_address"] = all_targets[0]
    elif config["farmer"]["pea_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['pea_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "pea_target_address" not in config["pool"]:
        print(f"Setting the wee destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["pea_target_address"] = all_targets[0]
    elif config["pool"]["pea_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['pea_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root_path: Path):
    ...
    weed_ca_crt, weed_ca_key = get_weed_ca_crt_key()
    weed_ca_crt_path = ca_dir / "weed_ca.crt"
    weed_ca_key_path = ca_dir / "weed_ca.key"
    write_ssl_cert_and_key(weed_ca_crt_path, weed_ca_crt, weed_ca_key_path, weed_ca_key)
    ...
    weed_ca_crt, weed_ca_key = get_weed_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, weed_ca_crt, weed_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if weed_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return weed_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def weed_version_number() -> Tuple[str, str, str, str]:
...
def weed_minor_release_number():
    res = int(weed_version_number()[2])
    print(f"Install release number: {res}")
    return res


def weed_full_version_str() -> str:
    major, minor, patch, dev = weed_version_number()
...
def weed_init(
    ...
    """
    if os.environ.get("WEED_ROOT", None) is not None:
        print(
            f"warning, your WEED_ROOT is set to {os.environ['WEED_ROOT']}. "
            f"Please unset the environment variable and run weed init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Weed directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if WEED_ROOT is set, or if user has run weed init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'weed keys show --show-mnemonic-seed'")
