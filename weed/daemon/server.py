...
from weed.cmds.init_funcs import check_keys, weed_init
from weed.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from weed.daemon.keychain_server import KeychainServer, keychain_commands
from weed.daemon.windows_signal import kill
from weed.server.server import ssl_context_for_root, ssl_context_for_server
from weed.ssl.create_ssl import get_mozilla_ca_crt
from weed.util.weed_logging import initialize_logging
from weed.util.config import load_config
from weed.util.json_util import dict_to_json_str
from weed.util.keychain import (
    Keychain,
...
from weed.util.service_groups import validate_service
from weed.util.setproctitle import setproctitle
from weed.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Weed.")
    quit()
...
service_plotter = "weed plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "weed": "weed",
        "weed_wallet": "start_wallet",
        "weed_full_node": "start_full_node",
        "weed_harvester": "start_harvester",
        "weed_farmer": "start_farmer",
        "weed_introducer": "start_introducer",
        "weed_timelord": "start_timelord",
        "weed_timelord_launcher": "timelord_launcher",
        "weed_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("weed daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered WEED_ROOT
    os.environ["WEED_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with WEED_ROOT: {os.environ['WEED_ROOT']}")
    ...
    if service_command == "weed_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in weed_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    weed_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("weed_daemon")
