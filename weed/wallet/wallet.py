...
from weed.wallet.puzzles.puzzle_utils import (
...
from weed.wallet.secret_key_store import SecretKeyStore
from weed.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_weed(self, weed_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if weed_amount < 0:
            utxos = await self.select_coins(abs(weed_amount), exclude)
        else:
        ...
        weed_amount = spend_value + weed_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": weed_amount}]
