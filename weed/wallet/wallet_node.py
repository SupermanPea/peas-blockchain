...
from weed.daemon.keychain_proxy import (
    KeychainProxy,
...
from weed.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from weed.protocols import wallet_protocol
from weed.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from weed.protocols.protocol_message_types import ProtocolMessageTypes
from weed.protocols.wallet_protocol import (
...
from weed.server.node_discovery import WalletPeers
...
from weed.util.ints import uint32, uint128
from weed.util.keychain import Keychain
from weed.util.lru_cache import LRUCache
from weed.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from weed.util.path import mkdir, path_from_root
from weed.wallet.block_record import HeaderBlockRecord
from weed.wallet.derivation_record import DerivationRecord
from weed.wallet.settings.settings_objects import BackupInitialized
from weed.wallet.transaction_record import TransactionRecord
from weed.wallet.util.backup_utils import open_backup_file
from weed.wallet.util.wallet_types import WalletType
from weed.wallet.wallet_action import WalletAction
from weed.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'weed keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSWeedConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSWeedConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSWeedConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSWeedConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSWeedConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSWeedConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
