...
ROOT = pathlib.Path(importlib.import_module("weed").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `weed` file name (which is the executable)
    and the `weed` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `weed` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("weed-blockchain"))[0]
...
# TODO: collapse all these entry points into one `weed_exec` entrypoint that accepts the server as a parameter

entry_points = ["weed.cmds.weed"] + [f"weed.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "weed.util.bip39"])

if THIS_IS_WINDOWS:
    weed_mod = importlib.import_module("weed")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/weed/util/english.txt", "weed/util"))
datas.append((f"{ROOT}/weed/util/initial-config.yaml", "weed/util"))
datas.append((f"{ROOT}/weed/wallet/puzzles/*.hex", "weed/wallet/puzzles"))
datas.append((f"{ROOT}/weed/ssl/*", "weed/ssl"))
...
add_binary("weed", f"{ROOT}/weed/cmds/weed.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/weed/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/weed/server/start_{server}.py", COLLECT_ARGS)
